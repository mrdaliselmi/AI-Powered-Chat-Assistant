...
if len(list(os.listdir(config["save_path"]))) > 0:
    tokenizer = AutoTokenizer.from_pretrained(
        os.path.join(config["save_path"], os.listdir(config["save_path"])[0])
    )
...
def tokenize_function(examples):
    ...
    inputs = [
        f"{instruction_k} [CONTEXT] {' EOS '.join(dialog)} [KNOWLEDGE] {knowledge}"
        if knowledge != ""
        else f"{instruction_nk} [CONTEXT] {' EOS '.join(dialog)}"
        for dialog, knowledge in zip(examples["dialog"], examples["knowledge"])
    ]
    ...
    labels = tokenizer(
        text_target=targets, max_length=config["length"], truncation=True
    )
...
class f1:
    def compute(self, predictions, references, type="marco"):
        ...
        for i in range(len(predictions)):
            ...
            e = (1e-5) / (
                len(" ".join(predictions[i]).split())
                + len(" ".join(references[i][0]).split())
                + 2
            )
            precisions.append(p)
            recalls.append(r)
